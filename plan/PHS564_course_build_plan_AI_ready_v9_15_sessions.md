...
**Format:** 15 sessions (up to 120 min each) + optional recitations/labs
...
## Lecture 12 — Target trial emulation I: protocol, time zero, and causal contrasts (MIMIC-IV Demo)
### Assets to generate (slides + notebook + homework)
- Slides: `slides/L12_target_trial_protocol.md`
- Notebook (student): `notebooks/L12_target_trial_protocol_student.ipynb` (checklists + partially-filled protocol builder)
- Notebook (solution, instructor): `instructor_solutions/L12_target_trial_protocol_solution.ipynb`
- Homework prompt (team): `assignments/HW12_target_trial_protocol_v1.md`
- Homework solutions (private): `instructor_solutions/HW12_target_trial_protocol_v1_solution.md`
- Exit poll: `polls/P12_exit_poll.md`

### Learning goals
- Translate an informal clinical question into a **complete target trial protocol**:
  eligibility, time zero, treatment strategies, assignment, follow-up, outcomes, causal contrast, analysis plan.
- Identify and prevent common protocol failures: **immortal time**, post-treatment conditioning, misaligned time zero, inappropriate adjustment.
- Decide *a priori* what you will estimate (ITT vs per-protocol; RD vs RR; risk vs hazard).

### In-class structure (highly structured)
1. **15 min:** recap from Lecture 11 (MSM weights + why time-varying confounding breaks naive adjustment).
2. **25 min:** target trial template walkthrough (protocol elements + “time zero” rules).
3. **35 min:** teams draft **Protocol v1** using the notebook’s guided prompts (you circulate).
4. **20 min:** “red-team” exercise: each team swaps protocols and identifies 2 likely biases + 1 positivity risk.
5. **10 min:** debrief + what must be fixed before workshop.

### Colab notebook idea
**Notebook 12:** “Protocol builder + bias checklist”
- Fill-in blocks: eligibility, time zero, strategies, outcomes, contrast, causal estimand.
- Auto-generate a `protocol.md` file and a 1-slide “study schematic” figure.
- Include a “bias flags” section that students must complete (immortal time, selection, positivity).

### Homework (team)
- Submit **Protocol v1** (≤3 pages) + a short “bias audit” section:
  - one paragraph on exchangeability strategy (what confounders and why),
  - one paragraph on positivity/overlap risk,
  - one paragraph on time zero and censoring.

---

## Lecture 13 — Target trial emulation II: hands-on workshop (analysis pipeline on MIMIC-IV Demo)
### Assets to generate (slides + workshop notebook + homework)
- Slides: `slides/L13_target_trial_workshop.md`
- Workshop notebook (student): `notebooks/L13_target_trial_workshop_student.ipynb` (mostly complete; *fill-in-the-blanks*)
- Workshop notebook (solution): `instructor_solutions/L13_target_trial_workshop_solution.ipynb`
- Homework prompt (team): `assignments/HW13_target_trial_analysis_pipeline.md`
- Solutions (private): `instructor_solutions/HW13_target_trial_analysis_pipeline_solution.md`
- Exit poll: `polls/P13_exit_poll.md`
...
### In-class structure
1. **10 min:** recap from Lecture 12 (protocol elements; time zero).
2. **20 min:** instructor demo: where things fail in EHR (definitions, missingness, overlap).
3. **70 min:** team workshop in notebook (guided):
   - load instructor-provided Demo extract,
...
4. **20 min:** 3–4 teams present “what broke” + how they fixed it.

### Colab notebook idea
**Notebook 13:** “End-to-end emulation on Demo extract”
- Nearly complete pipeline; students fill in:
  - covariate lists (baseline vs time-varying),
  - model specifications,
  - diagnostics thresholds (e.g., truncation at 1st/99th percentile),
...
## Lecture 14 — Robustness and sensitivity: unmeasured confounding, negative controls, and pragmatic checks
### Assets to generate (slides + notebook + homework)
- Slides: `slides/L14_sensitivity_negative_controls.md`
- Notebook (student): `notebooks/L14_sensitivity_negative_controls_student.ipynb`
- Notebook (solution): `instructor_solutions/L14_sensitivity_negative_controls_solution.ipynb`
- Homework prompt (team): `assignments/HW14_sensitivity_appendix.md`
- Solutions (private): `instructor_solutions/HW14_sensitivity_appendix_solution.md`
- Exit poll: `polls/P14_exit_poll.md`

### Learning goals
- Quantify how strong unmeasured confounding must be to explain away an effect (at least one method):
  **E-value** (for RR/OR) or a simple bias-factor sensitivity.
...
### Colab notebook idea
**Notebook 14:** “Sensitivity toolkit (minimal, interpretable)”
- Compute E-values for an RR estimate + CI.
- Simple tipping-point grid (bias factor) with a heatmap.
- A “negative controls brainstorming” worksheet (structured, not open-ended).

### Homework (team)
- Add a `robustness.md` appendix to the project repo:
  - one sensitivity analysis (E-value or bias-factor),
  - one negative control proposal + justification,
  - one “what would change our conclusion?” paragraph.

### Optional mini-module (asynchronous): Instrumental variables (IV) — **still not core**
- Short handout + optional notebook:
...
## Lecture 15 — Capstone: project presentations + course synthesis
### Assets to generate (slides + presentation template + poll)
- Slides: `slides/L15_capstone_presentations.md`
- Presentation template (student): `project/presentation_template.pptx` (or `presentation_template.md`)
- Exit poll: `polls/P15_exit_poll.md` (course feedback + self-assessment)

### In-class structure
- **10 min:** recap from Lecture 14 (what “robust” results look like).
- **75–90 min:** team presentations (5–6 min each + 2 min Q&A; adapt to enrollment).
- **15 min:** synthesis: common pitfalls seen across projects and “how to do this in real papers.”
- **Remaining time:** final exam logistics and expectations.

### Final deliverables (team)
- `project/protocol.md` (final)
- `project/notebooks/analysis.ipynb` (runs end-to-end on Demo; full MIMIC optional)
- `project/report.md` (≤6 pages; estimand, assumptions, methods, results, limitations)
...
A GitHub repository that includes, for **each lecture L01–L15**:
